...
import { insert, select, update } from './api/client'
...
export async function sendEmailViaResend(to, subject, htmlBody, emailType = 'general', relatedData = {}) {
  try {
    ...
    try {
      await insert('email_notifications', {
        user_id: userId,
...
export async function queueEmailNotification(emailNotification) {
  try {
    ...
    const data = await insert('email_notifications', {
      user_id: user.id,
...
export async function queueInvoiceEmail(invoiceId, recipientEmail) {
  try {
    ...
    const invoices = await select('project_invoices', {
    ...
    const projects = await select('projects', {
      filters: { 'id__eq': invoice.project_id },
