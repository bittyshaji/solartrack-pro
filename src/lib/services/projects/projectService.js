 ...
 * Handles all project CRUD operations via the centralized API layer
...
export async function getProjects(filters = {}) {
  try {
    ...
    if (filters.stage) {
      builder.filter('stage', 'eq', filters.stage)
    }
...
export async function getProjectById(id) {
  try {
    ...
    logger.error('Error fetching project', { projectId: id, error: err.message })
    return null
  }
}
...
export async function createProject(projectData) {
  try {
    ...
    const data = await insert('projects', insertData)
